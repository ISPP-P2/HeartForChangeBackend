...
public class TaskServiceImpl implements TaskService {
	...
	public TaskDTO getById(String token, Long id) {
		...
		if (!optionalTask.isPresent()) {
			...
			if (ong != null && optionalTask.get().getOng().getId() == ong.getId()) {
				...
				return tasktDTO;
			} else if (person != null && person.getRolAccount() == RolAccount.VOLUNTEER
					&& optionalTask.get().getOng().getId() == person.getOng().getId()
					&& optionalTask.get().getType() == TaskType.ACTIVIDAD) {
				Task task = optionalTask.get();
				TaskDTO tasktDTO = new TaskDTO(task);
				return tasktDTO;
			} else
	...
	public void deleteActivity(String token, Long id) {
		...
		if (task.isPresent()) {
			if (task.get().getType().equals(TaskType.ACTIVIDAD)) {
				...
				throw new IllegalArgumentException("This task is not a Actividad!");
	...
	public void deleteCurso(String token, Long id) {
		...
		if (task.isPresent()) {
			if (task.get().getType().equals(TaskType.CURSO)) {
				...
				throw new IllegalArgumentException("This task is not a Curso!");
	...
	public void deleteTaller(String token, Long id) {
		...
		if (task.isPresent()) {
			if (task.get().getType().equals(TaskType.TALLER)) {
				...
				throw new IllegalArgumentException("This task is not a Taller!");
		...
		if (ong != null) {
			...
			if (optionalTask.isPresent() && ong.getTasks().contains(optionalTask.get())) {
				for (Attendance attendance : optionalTask.get().getAttendance()) {
					BeneficiaryDTO volunteerDTO = new BeneficiaryDTO(
							beneficiaryRepository.findById(attendance.getPerson().getId()).get(),
	...
	public List<AttendanceDTO> getAllAttendancesOngByVolunteer(String token, Long id) {
		...
		if (volunteer.isPresent()) {
			...
			if (volunteer.get().getOng().equals(loggedOng)) {
				...
				throw new UsernameNotFoundException(
						"You cannot get information about volunteers who do not belong to your ONG.");
			}
			// Logged as volunteer
	...
	public List<AttendanceDTO> getAllAttendancesByBeneficiary(String token, Long id) {
		...
		if (beneficiary.isPresent()) {
			...
			if (beneficiary.get().getOng().equals(loggedOng)) {
				...
				throw new UsernameNotFoundException(
						"You cannot get information about beneficiaries who do not belong to your ONG.");
			}
			// Logged as volunteer
	...
	@Override
	public List<AttendanceDTO> getPetitionsByTask(String token, Long id) {
		String username = jwtUtils.getUserNameFromJwtToken(token);
		Ong ong = ongRepository.findByUsername(username);
		List<AttendanceDTO> attendances = new ArrayList<>();
		// Logged as ONG
		if (ong != null) {
			Optional<Task> optionalTask = taskRepository.findById(id);
			List<Attendance> waitingAttendances = attendanceRepository.findByTaskIdAndState(id, PetitionState.ESPERA);
			// If the task you are searching for is part of your ONG
			if (optionalTask.isPresent() && ong.getTasks().contains(optionalTask.get())) {
				for (Attendance attendance : waitingAttendances) {
					AttendanceDTO attendanceDTO = new AttendanceDTO(attendance);
					attendances.add(attendanceDTO);
				}
			} else {
	...
	@Override
	public List<AttendanceDTO> getAttendancesByActivity(String token, Long id) {
		String username = jwtUtils.getUserNameFromJwtToken(token);
		Ong ong = ongRepository.findByUsername(username);
		List<AttendanceDTO> attendances = new ArrayList<>();
		// Logged as ONG
	...
	@Override
	public List<AttendanceDTO> getAllAttendancesByWorkshop(String token, Long id) {
		String username = jwtUtils.getUserNameFromJwtToken(token);
		Ong ong = ongRepository.findByUsername(username);
		List<AttendanceDTO> attendances = new ArrayList<>();
		// Logged as ONG
	...
	@Override
	public List<AttendanceDTO> getAllAttendancesByCourse(String token, Long id) {
		String username = jwtUtils.getUserNameFromJwtToken(token);
		Ong ong = ongRepository.findByUsername(username);
		List<AttendanceDTO> attendances = new ArrayList<>();
		// Logged as ONG
	...
	@Override
	public AttendanceDTO getPetitionStateByONG(String token, Long idPerson, Long idTask) {
		String username = jwtUtils.getUserNameFromJwtToken(token);
		Ong ong = ongRepository.findByUsername(username);
		if (ong != null) {
